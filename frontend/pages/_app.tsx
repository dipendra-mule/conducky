...
const MyApp: React.FC<AppProps> = ({ Component, pageProps }) => {
  ...
  useEffect(() => {
    ...
    if (router.isReady && eventSlug && typeof eventSlug === 'string' && eventSlug !== '[eventSlug]') {
      logger.debug('Fetching event data for slug', { eventSlug, context: 'app_startup' });
      fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/events/slug/${eventSlug}`)
        ...
        .catch((error) => {
          logger.warn('Failed to fetch event details', { 
            error: error.message, 
            eventSlug, 
            context: 'app_startup' 
          });
      ...
      fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/events/slug/${eventSlug}/users`, { credentials: "include" })
        ...
        .catch((error) => {
          logger.warn('Failed to fetch event users', { 
            error: error.message, 
            eventSlug, 
            context: 'app_startup' 
          });
